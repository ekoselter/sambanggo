...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="post" action="{{ url('') }}/dashboard/kategoris/{{ $category[0]->slug }}" class="mb-5" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
                <label for="name" class="form-label">Nama Kategori</label>
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" required autofocus value="{{ old('name', $category[0]->name) }}">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" required value="{{ old('slug', $category[0]->slug) }}">
                @error('slug')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
